...
def generate_launch_description():
    ...
    # ── File paths ──────────────────────────────────────────────────────────
    urdf_file_path  = os.path.join(package_share, 'urdf',   'diff_robot_classic.urdf')
    ...
    # ── 2b. Static TF: map → odom (identity) ─────────────────────────────
    # Required so RViz2 can resolve the 'map' Fixed Frame.
    # This demo has no SLAM/AMCL, so map and odom are coincident at start.
    map_to_odom_tf = Node(
        package='tf2_ros',
